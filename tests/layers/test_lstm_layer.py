...
def lstm_layer_test():
    ...
    return lstm_layer.get_outputs(), lstm_layer_batch.get_outputs(), hidden_layer_size, batch_size


def test_lstm_layer():
    lstm_layer_out, lstm_layer_batch_out, hsize, bsize = lstm_layer_test()
    shape = lstm_layer_out.get_shape()
    assert len(lstm_layer_out.get_shape()) == 3
    # Checking if hidden size is accurate for non batched inputs
    assert int(shape[2]) == hsize
    shape = lstm_layer_batch_out.get_shape()
    assert len(lstm_layer_batch_out.get_shape()) == 4
    # Checking if hidden size is accurate if input and output are batched
    assert int(shape[3]) == hsize
    # Checking if batch size is accurate
    assert int(shape[0]) == bsize

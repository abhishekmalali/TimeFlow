...
def RMSE(predicted_tensor, response_tensor):
    ...
    """
    return tf.reduce_mean(tf.pow(tf.subtract(predicted_tensor, response_tensor), 2))
...
def R2(predicted_tensor, response_tensor):
    ...
    rse = tf.reduce_mean(tf.pow(tf.subtract(predicted_tensor, pred_mean), 2))
    mse = tf.reduce_mean(tf.pow(tf.subtract(predicted_tensor, response_tensor), 2))
    R2 = 1 - tf.div(mse, rse)
    return R2
